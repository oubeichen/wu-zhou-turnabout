    ...
    if (state.screen === "trial") {
      ...
      if (statement.unlockStatementId && !progress.unlockedStatements.includes(statement.unlockStatementId)) {
        return {
          id: "trial-hidden",
          title: "堵住的话",
          body: "他这话像楔子卡在整段证词里。一松开，后头压着的东西就全露出来了。",
          steps: ["当前句", "追问", "新证词"],
        };
      }
      ...
      if (statementHasAnswer(statement) && !selectedRecordLabel(caseData)) {
        ...
        return {
          ...
          body: trialDeduction
            ? `庭前对照早把两份记录压到了一处：${trialDeduction.deduction.text} 他再硬撑下去，露出来的就不只是巧合。`
            : statement.answerProfile
              ? "这处破绽不落在纸上，落在人身上。这个名字一摆出来，他就再难装成旁观者。"
  ...
  function caseOpeningStory(caseData) {
    ...
    const byCase = {
      ...
      "case-rebellion-box": {
        ...
        stakes: "真正让案子重起来的，不是第一声告发，而是纸张转手时被人添上的那层罪名。",
      },
      "case-urn": {
        kicker: "暗室余温",
        title: "供状太整齐，反而像刚从刑具旁擦干净。",
        body: "狄仁杰的签押端正得刺眼，暗室里的空瓮却还留着焦味。周兴把供词拍上案时，连看都不看那个瓮口。",
        stakes: "只看供状，赢的就是酷吏；刑具、笔迹和副本一摆出来，自愿二字自己会裂开。",
      },
      "case-half-hour-coup": {
        kicker: "夜门半小时",
  ...
  function caseSourceStoryItems(caseId) {
    const byCase = {
      "case-empress-seat": [
        ...
        { title: "诏稿遮住谁", note: "反对者的名字被墨盖住，新后的称号却留得干干净净。像是结局早写好了，只差一声哭来垫背。" },
        { title: "第一刀落下", note: "风波一变成惩罚，就能看出谁被推出去受罪，谁又稳稳站在后面等着得利。" },
        { title: "后位空出缺口", note: "哭声、诏稿和沉默一路接下去，后位就不是自然空出来的了，而是被人一步步腾开的。" },
      ],
      "case-crown-shadow": [
